...
class PurchaseInvoiceViewer(tk.Toplevel):
    def __init__(self, parent, invoice_id, supplier_info, items, subtotal, taxes, total):
        ...
        tk.Label(scrollable_frame, text="RIF: J-40339817-8", 
                font=("Arial", 12)).pack()
        tk.Label(scrollable_frame, text="FACTURA DE COMPRA", 
                font=("Arial", 16, "bold"), fg="#333").pack(pady=5)
        
        # Separador
        ttk.Separator(scrollable_frame).pack(fill="x", padx=20, pady=5)
        
        # Información de factura y proveedor en dos columnas
        ...
        tk.Label(left_frame, text=f"Fecha: {datetime.now().strftime('%d/%m/%Y %H:%M')}", 
                font=("Arial", 10)).pack(anchor="w")
        tk.Label(left_frame, text="Tipo: Compra", 
                font=("Arial", 10)).pack(anchor="w")
    ...
    def print_invoice(self):
        ...
        messagebox.showinfo("Imprimir", "La factura de compra se enviará a la impresora", parent=self)