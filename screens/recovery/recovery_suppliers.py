...
class RecoverySuppliers(tk.Frame):
    ...
    def configure_ui(self) -> None:
        ...
        title_label = CustomLabel(
            header_frame,
            text="Recuperación de Proveedores",
            font=("Arial", 20, "bold"),
            fg="white",
            bg="#4a6fa5"
        )
    ...
    def enable_supplier(self) -> None:
        ...
        response = messagebox.askyesno(
            "Confirmar", 
            f"¿Está seguro que desea habilitar el proveedor '{company_name}'?",
            parent=self
        )
        ...
        if response:
            try:
                ...
                if status_active:
                    Supplier.update_status(supplier_id, status_active['id'])
                    messagebox.showinfo("Éxito", "Proveedor habilitado correctamente", parent=self)
                    self.refresh_data()
                else:
                    messagebox.showerror("Error", "No se encontró el estado 'activo'", parent=self)
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo habilitar el proveedor: {str(e)}", parent=self)