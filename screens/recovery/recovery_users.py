...
class RecoveryUsers(tk.Frame):
    ...
    def enable_user(self) -> None:
        ...
        response = messagebox.askyesno(
            "Confirmar", 
            f"¿Está seguro que desea habilitar al usuario '{username}'?",
            parent=self
        )
        ...
        if response:
            try:
                ...
                if status_active:
                    User.update_status(user_id, status_active['id'])
                    messagebox.showinfo("Éxito", "Usuario habilitado correctamente", parent=self)
                    self.refresh_data()
                else:
                    messagebox.showerror("Error", "No se encontró el estado 'activo'", parent=self)
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo habilitar el usuario: {str(e)}", parent=self)