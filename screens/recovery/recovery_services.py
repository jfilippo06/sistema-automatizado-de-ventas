...
class RecoveryServices(tk.Frame):
    ...
    def configure_ui(self) -> None:
        ...
        title_label = CustomLabel(
            header_frame,
            text="Recuperación de Servicios",
            font=("Arial", 20, "bold"),
            fg="white",
            bg="#4a6fa5"
        )
        ...
        self.tree = ttk.Treeview(tree_frame, columns=(
            "ID", "Código", "Nombre", "Precio", "Descripción", "Estado"
        ), show="headings")
    ...
    def enable_service(self) -> None:
        ...
        response = messagebox.askyesno(
            "Confirmar", 
            f"¿Está seguro que desea habilitar el servicio '{service_name}'?",
            parent=self
        )
        
        if response:
            try:
                Service.enable(service_id)
                messagebox.showinfo("Éxito", "Servicio habilitado correctamente", parent=self)
                self.refresh_data()
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo habilitar el servicio: {str(e)}", parent=self)